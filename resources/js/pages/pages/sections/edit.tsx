...
interface GridItem {
    ...
    // Controls whether this individual grid item is visible publicly
    is_visible: boolean;
}
...
export default function Edit({
    ...
    const getExistingContent = (): Record<string, unknown> => {
        if (!section.content) {
            return {};
        }
    ...
    const getInitialContactContent = (): string => {
        if (section.type !== 'contact_form' || !section.content) {
            return '';
        }
    ...
    const {
        ...
        content: getInitialContactContent(),
    ...
    const [heroContent, setHeroContent] =
        useState<HeroFormContent>(() => {
            const content =
                section.type === 'hero'
                    ? existingContent as Partial<HeroFormContent>
                    : {};
    ...
    const [statsContent, setStatsContent] =
        useState<StatsContent>(() => {
            const content =
                section.type === 'stats'
                    ? existingContent as Partial<StatsContent>
                    : {};
            ...
            return {
                ...
                items: Array.isArray(content.items)
                    ? (content.items as Partial<StatItem>[]).map((item) => ({
                          value: item.value ?? '',
                          label: item.label ?? '',
                          suffix: item.suffix ?? '',
                          icon: item.icon ?? '',
    ...
    const [aboutContent, setAboutContent] =
        useState<AboutContent>(() => {
            const content =
                section.type === 'about'
                    ? existingContent as Partial<AboutContent>
                    : {};
        ...
        useState<VisionContent>(() => {
            const content =
                section.type === 'vision_mission'
                    ? existingContent as Partial<VisionContent>
                    : {};
    ...
    const [
        ...
        const content =
            section.type === 'certifications'
                ? existingContent as Partial<CertificationsContent>
                : {};
        ...
        return {
            ...
            ...content,
            items: Array.isArray(content.items)
                ? (content.items as Partial<CertificationItem>[]).map((item) => ({
                      name: item.name ?? '',
                      description: item.description ?? '',
                      image: item.image ?? '',
                      url: item.url ?? '',
    ...
    const [
        ...
        const content =
            section.type === 'global_presence'
                ? existingContent as Partial<GlobalPresenceContent>
                : {};

        return {
            ...defaultGlobalPresenceContent,
            ...content,
            offices: Array.isArray(content.offices)
                ? content.offices as OfficeItem[]
                : [],
        };
    });
    ...
    const [ctaContent, setCtaContent] =
        useState<CtaContent>(() => ({
            ...
            ...(
                section.type === 'cta'
                    ? existingContent as Partial<CtaContent>
                    : {}
            ),
        }));
    ...
    const [
        ...
        ...(
            section.type === 'content'
                ? existingContent as Partial<ContentSectionContent>
                : {}
        ),
    }));
    ...
    const [cardsContent, setCardsContent] =
        useState<CardsContent>(() => {
            const content =
                section.type === 'cards'
                    ? existingContent as Partial<CardsContent>
                    : {};
            ...
            return {
                ...
                ...content,
                items: Array.isArray(content.items)
                    ? (content.items as Partial<CardItem>[]).map((item) => ({
                          title: item.title ?? '',
                          description: item.description ?? '',
                          image: item.image ?? '',
                          icon: item.icon ?? '',
    ...
    const [gridContent, setGridContent] =
        useState<GridContent>(() => {
            const content =
                section.type === 'grid'
                    ? existingContent as Partial<GridContent>
                    : {};
            ...
            return {
                ...
                ...content,
                items: Array.isArray(content.items)
                    ? (content.items as Partial<GridItem>[]).map((item) => ({
                          title: item.title ?? '',
                          description: item.description ?? '',
                          image: item.image ?? '',
    ...
    const [faqContent, setFaqContent] =
        useState<FaqContent>(() => {
            const content =
                section.type === 'faq'
                    ? existingContent as Partial<FaqContent>
                    : {};
    ...
    const submit = (
        ...
        put(
            `/admin/pages/${page.id}/sections/${section.id}`
        );
                        ...
                        <div className="space-y-6 rounded-xl border border-pink-200 bg-pink-50/30 p-5">
                            <div>
                                ...
                                <p className="mt-1 text-sm leading-6 text-gray-600">
                                    Update image, logo, client and partner grid items. Each item can use a Media Library image or a direct image URL and can link to an external website.
                                </p>
                            </div>
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-pink-100 pt-5">
                                <div>
                                    ...
                                    <p className="mt-1 text-xs text-gray-500">
                                        Edit the image/logo, external image URL and website link for each item.
                                    </p>
                                </div>
                            ...
                            {gridContent.items.length === 0 && (
                                <div className="rounded-xl border border-dashed border-gray-300 bg-white px-6 py-8 text-center">
                                    ...
                                    <p className="mt-1 text-xs text-gray-500">
                                        Click “Add Grid Item” to create your first item.
                                    </p>
                                </div>
                            )}
                                                ...
                                                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 bg-gray-50/70 px-5 py-3">
                                                    <div>
                                                        ...
                                                        <p className="mt-0.5 text-xs text-gray-500">
                                                            Update the display image/logo and destination website.
                                                        </p>
                                                    </div>
                                                        ...
                                                        <div className="flex h-[150px] w-full items-center justify-center overflow-hidden rounded-xl border border-gray-200 bg-gray-50 p-3">
                                                            {previewUrl ? (
                                                                ...
                                                                <div className="px-4 text-center">
                                                                    ...
                                                                    <p className="mt-1 text-xs text-gray-400">
                                                                        Select a Media Library image or paste a direct Image URL.
                                                                    </p>
                                                                </div>
                                                            )}
                                                        </div>
                                                    ...
                                                    <div className="grid gap-4 md:grid-cols-2">
                                                        ...
                                                        <div className="md:col-span-2">
                                                            ...
                                                            <p className="mt-1.5 text-xs leading-5 text-gray-500">
                                                                Paste a direct public image URL. When this field is filled, it takes priority over the selected Media Library image.
                                                            </p>
                                                        </div>
                                                        ...
                                                        <div className="md:col-span-2">
                                                            ...
                                                            <p className="mt-1.5 text-xs leading-5 text-gray-500">
                                                                When provided, the complete public logo/card becomes clickable and opens this website.
                                                            </p>
                                                        </div>
                    ...
                    {data.type === 'faq' && (
                        ...
                        <div className="space-y-6 rounded-xl border border-emerald-200 bg-emerald-50/30 p-5">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-sm text-gray-600">
                                    Edit frequently asked questions and answers. These items are also used automatically for FAQPage Schema.org structured data.
                                </p>
                            </div>
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="mt-1 text-xs text-gray-500">
                                        Update existing questions or add new FAQ items.
                                    </p>
                                </div>
